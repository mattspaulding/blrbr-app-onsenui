...
function PagePlayer() {
	...
	var self = this, pl = this, sm = soundManager, // soundManager instance
	_event, vuDataCanvas = null, controlTemplate = null, _head = document.getElementsByTagName('head')[0], spectrumContainer = null,
	// sniffing for favicon stuff, IE workarounds and touchy-feely devices
	ua = navigator.userAgent, supportsFavicon = (ua.match(/(opera|firefox)/i)), isTouchDevice = (ua.match(/ipad|ipod|iphone/i)), cleanup;
	...
	_event = ( function() {
			...
			var old = (window.attachEvent && !window.addEventListener), _slice = Array.prototype.slice, evt = {
	...
	this.events = {
		...
		onload : function() {
			if (!this.loaded) {
				var oTemp = this._data.oLI.getElementsByTagName('a')[0], oString = oTemp.innerHTML, oThis = this;
	...
	this.updatePeaks = function() {
		var o = this._data.oPeak, oSpan = o.getElementsByTagName('span');
	...
	this.updateGraph = function() {
		...
		var sbC = this._data.oGraph.getElementsByTagName('div'), scale, i, offset;
		if (pl.config.useWaveformData) {
			// raw waveform
			scale = 8;
			// Y axis (+/- this distance from 0)
			for ( i = 255; i--; ) {
	...
	this.resetGraph = function() {
		...
		var sbC = this._data.oGraph.getElementsByTagName('div'), scale = (!pl.config.useEQData ? '9px' : '17px'), nHeight = (!pl.config.fillGraph ? '1px' : '32px'), i;
		for ( i = 255; i--; ) {
	...
	this.handleClick = function(e) {
		...
		var o = self.getTheDamnTarget(e), sURL, soundURL, thisSound, oControls, oLI, str;
		if (!o) {
			return true;
		}
		if (self.dragActive) {
	...
	this.createVUData = function() {
		...
		var i = 0, j = 0, canvas = vuDataCanvas.getContext('2d'), vuGrad = canvas.createLinearGradient(0, 16, 0, 0), bgGrad, outline;
	...
	this.init = function(oConfig) {
		...
		this.cssBase = [];
		// optional features added to ul.playlist
		...
		if (self.config.autoStart) {
			...
			var num = pl.getByClassName('playlist', 'ul')[0].getElementsByClassName('clip').length - 1;
			pl.handleClick({
				target : pl.getByClassName('playlist', 'ul')[0].getElementsByClassName('clip')[num]
			});
		}
